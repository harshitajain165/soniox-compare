...
export const ProviderCost = ({
  ...
  // Speaking the translation costs more than just writing it.
  const ratePerHour = pricing ? getPricePerHour(pricing, settings.mode) : 0;
  ...
  const cost = (activeMs / 3600000) * ratePerHour;

  const costLabel = `~${formatEstimatedCost(cost)}`;
  const priceLabel = pricing ? formatPricePerHour(ratePerHour) : "n/a";
