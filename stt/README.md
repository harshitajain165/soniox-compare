...
Nine providers, registered in `PROVIDER_MAP` (`main.py`). Model IDs are the wire values from `config.py` — except Google, whose model id lives in its provider module because the Gemini Live session and the feature matrix must agree on it. Each provider module reports its model in `get_available_features()`.
