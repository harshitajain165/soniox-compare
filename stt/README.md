...
Nine providers, registered in `PROVIDER_MAP` (`main.py`). Model IDs are the wire values from `config.py`; each provider module reports the same model in `get_available_features()`.
...
| `GOOGLE_CREDENTIALS_JSON` | Google — full service-account JSON as the value; falls back to `./credentials-google.json` (git-ignored) for local dev |
